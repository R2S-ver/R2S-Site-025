...
想法很简单：用手机控制一颗 LED。不搞蓝牙、不装 App——就用 ESP32 自己跑一个网页，家里 WiFi 下的任何设备打开浏览器就能用。页面上放个滑块，拖动的时候 LED 亮度跟着实时变化。做下来发现这其实是个挺好的小练习，把 WiFi、HTTP 和 PWM 这三样 IoT 项目里必现的东西串在了一起。
