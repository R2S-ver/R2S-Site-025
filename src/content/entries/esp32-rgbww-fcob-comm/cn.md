...
在做任何有意思的效果之前，我得先把最基本的东西确认清楚：ESP32 能不能跟这条 FCOB 灯带正常通信、数据线接对了没有、颜色通道的顺序我有没有搞错。FCOB 灯带跟普通可寻址 LED 有点不一样;它用的是一整条连续的荧光涂层而不是一颗颗分立的灯珠，所以发光特别均匀柔和。但通信协议还是那个 NeoPixel/WS2812 风格的单线接口，软件这边倒是熟门熟路。
...
先假设了 150 个控制点;其实当时还不确定这条灯带到底有多少个（后面才搞清楚），反正是通信测试，写个够大的数就行。这里主要想验证两件事：一是 `NeoGrbwFeature` 的颜色顺序（库里要求的是红、白、绿、蓝，不是常见的 R、G、B、W），二是 `NeoEsp32Rmt0800KbpsMethod` 能不能通过 ESP32 的 RMT 外设稳定输出 800 kbps 的信号。这两样要是搞错了，要么灯根本不亮，要么随机乱闪颜色，然后你就得对着接线怀疑人生半小时。
