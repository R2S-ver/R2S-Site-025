...
起点是现款货箱。它能用，但积攒了多年的妥协。重新设计的任务书收敛为五个针对性改动：

| 改动 | 设计依据 |
| --- | --- |
| 单侧盖板取代双侧盖板 | 一个盖子比两个盖子打开更方便，也只需要一把锁，而不是两把不同的钥匙。新设计只从一侧开合，并且朝右：自行车停靠在街道旁时，油漆工在人行道一侧打开货箱，而不是面向左侧的车流；装卸货更安全。 |
...
| 使用更轻的材料 | 该结实的地方结实，其他地方用更轻的材料：载荷由型钢框架承担，外壳用 [Alupanel](https://kunststofplatenshop.nl/alupanel-platen/) 铝复合板让整车轻便；类似汽车 A 柱与车身外壳的分工；从而容纳更多货物。 |
...
在开始任何 CAD 工作之前，重新设计建立在三方面输入之上：对市场上现有方案的研究、对部件选项的可行性与成本对比，以及对这个货箱的使用者；油漆工客户本人的直接访谈。
...
客户直接回答了我们的提问；下面的回答直接左右了多个设计选择：

- <strong style="color:var(--accent)">盖板重量</strong>：SolidWorks 实测约 10–12 kg；气弹簧的选型从这里开始。
- <strong style="color:var(--accent)">开盖幅度</strong>：用户希望盖板开得越大越好。
...
- 同类设计中的盖板使用较薄的铝板。
- 盖板值得考虑使用气弹簧；盖板上方还要挂一个 15 公斤的梯子。
- 货箱高度约 75 cm。
- 车把高度必须与 ZOEM 标准车把一致。
- 货运自行车驾驶舱本身重量可观。
...
<div class="tab-panel active"><h4>金属腐蚀</h4><p>当铝材与不锈钢紧固件接触时会发生腐蚀；这就是电偶腐蚀。它发生在两种材料（阳极与阴极）通过电解质（雨水就足够了）相互接触时。</p><p><strong>最终选择：</strong>钢框架与铝件整体粉末喷涂。流程：工厂加工钣金并在工厂焊接（我们的工坊不具备焊接铝材的技术）→ 运回组装测试 → 打孔，此后不能再更改 → 发往另一家工厂粉末喷涂 → 运回最终组装。产品的氧化腐蚀控制在预估使用寿命之内。</p><p class="src">Source: 1. Why Can't You Use Stainless Steel and Aluminum Together?</p></div>
...
<div class="tab-panel"><h4>气弹簧</h4><p>供应商对比：Amatec（标准固定眼式气弹簧）、Tevema、Gasveerwinkel（全系列在线销售，含选型配置器）、Spring Masters（现货供应）。同时考虑了球头铰链（kogelscharnier）作为替代安装方案。</p><p><strong>最终选择：</strong>250N 气弹簧 ×2，托住 10–12 kg 盖板的全程行程；在最终装配时重新选定并实测验证（制造阶段）。</p><p class="src">Source: 1. Gasveerwinkel 2. Gasveerexpert 3. Amatec 4. Tevema 5. Spring Masters</p></div>
...
<div class="tab-panel"><h4>防水密封</h4><p>防水由三层构成：防水铆钉、接缝处的玻璃胶密封，以及框架本身的结构设计；盖板边缘做成类似屋顶流水槽的造型，雨水顺着边缘流走，而不是灌进货仓。</p></div>
...
- <strong style="color:var(--accent)">气弹簧选型</strong>：盖板大约重 10–12 kg。我们测试了能舒适托起盖板的弹簧，同时尽量减小关闭状态下的机械疲劳；那是弹簧带载度过大部分寿命的姿势。
- <strong style="color:var(--accent)">实地走访 [Lino Metaalwaren](https://lino-metaal.nl/)</strong>：专门去了当地的五金件供应商，在确定自家方案之前，先亲手了解铰链设计的各种选项。
...
我和同伴一起焊接了框架，切割了外壳，拆下旧货箱并组装了新货箱，途中在多个阶段进行了气弹簧测试。最终装配时重新选定了气弹簧：250N ×2，托住盖板的全程行程，并且留足了安全余量。
...
这是第一个"流程终点是客户骑上成品"的项目；不是分数，也不是躺在抽屉里的原型。

- <strong style="color:var(--accent)">钣金是一种完全不同的设计语言</strong>：折弯半径、焊缝、喷塑公差，这些几何约束远早于审美进入对话。在 SolidWorks 里画图和用钢铁把它造出来，是两套完全不同的交流方式。
- <strong style="color:var(--accent)">设计是平衡的语言</strong>：每一个零件决策；铰链、气弹簧、板材厚度；都要在成本、使用寿命、可加工性、交期和用户的日常使用体验之间权衡。纸面上最好的零件，未必是产品里最好的零件。
- <strong style="color:var(--accent)">两人小组需要共享的现实</strong>：只有两个人，意味着每一张草图、每一个决策都必须立刻能被对方读懂。文档不再是负担，而是我们保持同步的唯一方式。
...
*特别感谢 [Lino Metaalwaren](https://lino-metaal.nl/) 赞助本项目的不锈钢钢琴铰链。*
