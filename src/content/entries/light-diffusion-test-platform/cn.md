...
- <strong style="color:var(--accent)">用校准的数字控制取代手动PWM控制</strong>：目前的设置依赖于手动调节的PWM控制器，虽然足以满足探索性测试的需求，但会导致测量间光强度存在一定差异。微控制器可以生成精确定义的PWM信号并存储可重复的强度设置，从而确保在不同测试中实现相同的照明条件。这将提高数据的可靠性，并使材料间的比较更加严谨。
