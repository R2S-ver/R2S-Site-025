...
前面两个实验攒的东西在这里汇合了。FCOB 灯带已经能跟 ESP32 正常通信，WiFi 网页服务器也知道怎么搭了。下一步很自然——做一个像样的取色器。不是几个 RGB 滑块凑合一下，而是能拖拽的真色环，灯带颜色实时跟着变。
...
想通了之后这个白光提取其实特别简单。任何 R、G、B 三个通道都有值的颜色——灰色、粉彩、暖色调——里面都包含了一个"白光分量"，大小等于三个通道中的最小值。把这个分量减出来，交给专门的暖白灯珠去发光，剩下的 RGB 值就是你想要的那个纯色调。不拆开的话，像 `(255, 200, 150)` 这种颜色看起来就会发灰发白，因为 RGB 灯珠同时要兼顾色调和白光——LED 干这个并不擅长。拆开之后颜色干净很多，肉眼可见的差别。
