...
>
> - 最后更新：2026-08-16 v1.3（v1.2 美化子集批次：三族字体/标题 display 字体、hero 字距 -2px、暗角加强、列表读数条、notes 滚动淡入、页面 CRT 过渡（ClientRouter）、卡片 CRT 开机动画（每次完整加载触发一次）、about introTitle 四行 + 彩蛋打字提速、404 PM5544 测试卡。磁带盒装置/卡片标签条/音效/glitch 等未采用。v1.3：首页 Hero ≥2328px 宽屏线性步进（2560px 时 110% 等效，2K@100% 对齐原 110% 观感）。）
...
Layout 负责：head 全套（主题守卫内联脚本→防闪烁、charset/viewport/color-scheme/theme-color×2、Google Fonts JetBrains Mono + Saira Condensed 500-900 + VT323、自托管得意黑 `/fonts/dyh/result.css`、favicon、canonical、hreflang+x-default、og:*/twitter:*、**JSON-LD WebSite**、`<ClientRouter />` 页面过渡）；body 结构（skip-link、`#mobile-notice` 移动端提示、`#neural-canvas` 粒子画布、`<Navbar/>` + `<main#main-content/>`(slot) + `<Footer/>`、`<ScrollMeter/>`、`#crt-overlay` + `#tube-vignette` 背景层、粒子/主题切换/mobile-notice/滚动 reveal/CRT 开机五个脚本）。`global.css` 在此以 frontmatter import 引入（唯一引入点）。
...
**首页专用（`src/components/home/`，仅 index.astro 与 zh/index.astro 用）：**
...
| `AboutPreview.astro` | OPERATOR 身份预览面板 |
...
**字体：** `--font-mono: "JetBrains Mono", "PingFang SC", "Microsoft YaHei", "Noto Sans SC", "Courier New", monospace`；`--font-body: Arial, Helvetica, "PingFang SC", "Microsoft YaHei", "Noto Sans SC", sans-serif`；2026-08 新增三族：`--font-display: "Saira Condensed"`（拉丁标题，§15 全局 h1-h3，art 页除外）、`--font-zh-display: "Smiley Sans Oblique"`（中文标题，自托管 `/fonts/dyh/`）、`--font-pixel: "VT323"`（终端/OSD/ScrollMeter）。hero h1 字距 -2px（用户要求比旧 -6px 拉开）。

**字号**（`--text-scale-*`，暗色定义亮色复用）：hero `clamp(80px,10vw,110px)`、display `clamp(36px,5vw,64px)`、h1 64、h2 48、h3 34、article-h1 42、article-h2 32、article-h3 24、body 18、sm 15、xs 13 等。Hero.astro 内有局部宽屏覆盖：`@media (min-width: 2328px)` 用 vw 线性 ramp 到 110% 等效（如 `--text-scale-hero: clamp(110px, 4.7266vw, 121px)`）——2560px（2K@100%）命中 121px，对齐原 2K@110% 观感；2K@110% 视口约 2327 CSS px 不触发，保持该档不变；1080p@100% 也不受影响。
